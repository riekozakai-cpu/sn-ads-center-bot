...
const testCases = [
  {
    name: 'テスト1: ヘルプ記事あり（ヘルプセンター検索経由）',
    query: '広告の入稿方法を教えてください',
    useSearch: true,
  },
  {
    name: 'テスト2: ヘルプ記事なし（参考情報なしで質問）',
    query: '存在しない機能について教えてください',
    useSearch: false,
  },
  {
    name: 'テスト3: URLを捏造しないか確認',
    query: 'アカウントの作成方法を教えてください',
    useSearch: true,
  },
];

async function runTests() {
  console.log('=== プロンプト動作確認テスト ===\n');

  for (const tc of testCases) {
    console.log(`\n${'='.repeat(60)}`);
    console.log(`📋 ${tc.name}`);
    console.log(`質問: ${tc.query}`);
    ...
    try {
      ...
      if (urls.length === 0) {
        ...
        urls.forEach(url => {
          ...
          console.log(`  ${inContext ? '✅' : '❌'} ${url} ${inContext ? '(入力に含まれる)' : '(入力に含まれない！)'}`);
        });
      }
    } catch (error) {
